...
export const createTodo = middyfy(async (event: any): Promise<APIGatewayProxyResult> => {
    try {
        const id = v4();
...
export const updateTodo = middyfy(async (event: any): Promise<APIGatewayProxyResult> => {
    const id = event.pathParameters.id;
    try {
